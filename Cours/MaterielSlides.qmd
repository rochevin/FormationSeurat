...
```{r}
library(nebula)
ifnb_neb1<-scToNeb(obj=ifnb,assay="RNA",id="donor_id",pred=c("stim"))
df1 = model.matrix(~stim, data=ifnb_neb$pred)
data_g = group_cell(count=ifnb_neb$count,id=ifnb_neb$id,pred=df1) # il faut grouper les cellules par indiv avant de faire nebula
...
DotPlot(ifnb,features=genetopNeb1$gene[1:10],group.by="stim")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
ifnb_neb2<-scToNeb(obj=ifnb[,which(Idents(ifnb)=="CD14 Mono")],assay="RNA",id="donor_id",pred=c("stim"))
...
```{r}
library(ggvenn)
ggvenn(list(DEseq2=rownames(genetopDEseq2),Nebula=genetopNeb2$gene,Wilcox=rownames(genetop_wilcox2)))
```
...
#I<-which(ifnb$seurat_annotations=="CD14 Mono")
#datatrain<-t(as.matrix(ifnb@assays$integrated$scale.data[,I]))
#dataRF<-data.frame(datatrain,
#                   Cl = as.factor(ifnb$stim[I]))
