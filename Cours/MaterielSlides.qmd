...
```{r}
#Select_auc<-markers_auc%>%
#    filter(myAUC>aucthresh)
#  Select_auc$gene<-rownames(Select_auc)
#  Select_wilcox<-markers_wilcoxon%>%
#    filter(p_val_adj < pvalthresh)
#  Select_wilcox$gene<-rownames(Select_wilcox)
#  Select<-merge(Select_auc,Select_wilcox,by="gene")
```
