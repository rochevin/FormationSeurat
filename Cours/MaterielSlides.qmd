...
```{r}
# pseudobulk
pseudo_ifnb2 <- AggregateExpression(ifnb[,which(ifnb$seurat_annotations=="CD14 Mono")], 
                                   assays = "RNA", 
                                   return.seurat = T, 
...
```{r}
#pseudo_ifnb2$celltype.stim <- paste(pseudo_ifnb2$seurat_annotations, pseudo_ifnb2$stim, sep = "_")
...
```{r}
library(nebula)
#ifnb_neb<-scToNeb(obj=ifnb,assay="RNA",id="donor_id",pred=c("stim"))
#df1 = model.matrix(~stim, data=ifnb_neb$pred)
#re=nebula(ifnb_neb$count,id=ifnb_neb$id,pred=df1)
```

```{r}
#mean(p.adjust(re$summary$`p_TreatmentNF:GenotypeWT`,method="BH")<0.01)
#mean(p.adjust(re$summary$`p_TreatmentNF`,method="BH")<0.01)
#mean(p.adjust(re$summary$`p_GenotypeWT`,method="BH")<0.01)
#mean(p.adjust(re$summary$`p_Replicate2`,method="BH")<0.01)
```


```{r}
#NebulaSumm<-re$summary
#NebulaSumm[,"padj_Inter"]<-p.adjust(NebulaSumm$`p_TreatmentNF:GenotypeWT`,method="BH")
#NebulaSumm[,"abslogFCInter"]<-abs(NebulaSumm$`logFC_TreatmentNF:GenotypeWT`)
#SelectNebulaInter<-NebulaSumm %>%
#  filter(padj_Inter <0.01 & abslogFCInter>1)%>%
#  arrange(desc(abslogFCInter))
```
